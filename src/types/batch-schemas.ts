...
export const BatchExecuteParamsSchema = z.object({
  commands: z.array(z.object({
    ...
    environment_variables: z.record(z.string()).optional().describe('Environment variables for this command'),
    timeout_seconds: z.number().int().min(1).max(3600).optional().describe('Individual timeout for this command'),
  })).min(1).max(10).describe('Array of commands to execute (max 10 for safety)'),
  execution_mode: z.enum(['sequential', 'parallel']).default('sequential').describe('How to execute the commands: sequential (one after another) or parallel (all at once)'),
  failure_strategy: z.enum(['stop_on_error', 'continue', 'best_effort']).default('stop_on_error').describe('What to do when a command fails'),
  batch_timeout_seconds: z.number().int().min(10).max(3600).default(300).describe('Overall timeout for the entire batch'),
  return_individual_outputs: z.boolean().default(true).describe('Return individual command outputs or just summary'),
});
...
export const BatchFileOperationParamsSchema = z.object({
  ...
  })).min(1).max(20).describe('Array of file operations to perform'),
  parallel: z.boolean().default(true).describe('Execute operations in parallel for better performance'),
  continue_on_error: z.boolean().default(true).describe('Continue processing other files if one fails'),
});
...
export const SystemOverviewParamsSchema = z.object({
  ...
  summary_only: z.boolean().default(false).describe('Return only summary statistics, not detailed lists'),
});
