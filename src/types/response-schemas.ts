...
// 共通の応答レベル制御
export const ResponseLevelSchema = z.enum(['minimal', 'standard', 'full']).default('standard').describe(
...
export const OutputFormatSchema = z.object({
  ...
  exclude_metadata: z.boolean().default(false).describe('Exclude metadata like timestamps, IDs for cleaner output'),
  exclude_empty_fields: z.boolean().default(true).describe('Remove fields with null/empty values from response'),
  compact_arrays: z.boolean().default(false).describe('Compact array outputs by showing only count and first few items'),
  summary_only: z.boolean().default(false).describe('Return only summary information instead of detailed data'),
});

// バッチ操作結果の集約
export const BatchResultFormatSchema = z.object({
  show_individual_results: z.boolean().default(true).describe('Include individual operation results'),
